...
import { isAbortError, waitForConfirm, waitForRetry, setErrorHandlerLogger } from './error-handler'
...
export async function runAgent(
  ...
  // 初始化子模块状态（幂等，可重复调用）
  if (services) {
    bootstrapAgentMemory(services)
  }

  // 加载项目授权级别（跟随项目持久化）
  if (projectId && services?.database) {
    loadAuthLevel(projectId, services.database)
  }
  ...
  while (round < MAX_TOOL_ROUNDS) {
    ...
    syncRuntimeToolPrompt(buildAllowedToolNamesForRequest())
