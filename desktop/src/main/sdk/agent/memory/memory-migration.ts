...
import type { MemorySnapshotEntry } from './snapshot'
import type { Logger, DatabaseService, NotesService } from '../services'
import { TACO_HOME } from '../paths'
import { readJsonArray } from './memory-utils'
import { normalizeTaskMemoryEntry } from './memory-normalize'
import { normalizeMemorySnapshotEntry } from './snapshot'
...
export function setMemoryMigrationLogger(logger: Logger) { _log = logger }
export function setMemoryMigrationDatabase(db: DatabaseService) { _db = db }
export function setMemoryMigrationNotes(notes: NotesService) { _notesService = notes }

function getDb(): DatabaseService {
  if (!_db) throw new Error('DatabaseService not initialized for memory-migration')
  return _db
}

function getNotes(): NotesService {
  if (!_notesService) throw new Error('NotesService not initialized for memory-migration')
...
export async function ensureLegacyMemoryDbBootstrap(): Promise<void> {
  ...
  legacyMemoryDbBootstrapPromise = (async () => {
    getDb().initMemoryDb('', undefined)
    if (!getDb().isMemoryDbEmpty('', undefined)) {
    ...
    for (const scopeKey of Array.from(scopeKeys).sort()) {
      ...
      const normalizedNotes = legacyNotes.map((item, index) => getNotes().normalize(item))
      ...
      if (normalizedNotes.length > 0) {
        getDb().importProjectNotes(scopeRef.workspace, normalizedNotes, scopeRef.projectId)
        importedNoteCount += normalizedNotes.length
      }
      if (normalizedActive.length > 0) {
        getDb().importTaskMemoriesByTier(scopeRef.workspace, 'active', normalizedActive, scopeRef.projectId)
        importedActiveCount += normalizedActive.length
      }
      if (normalizedArchive.length > 0) {
        getDb().importTaskMemoriesByTier(scopeRef.workspace, 'archive', normalizedArchive, scopeRef.projectId)
        importedArchiveCount += normalizedArchive.length
      }
      if (normalizedSnapshots.length > 0) {
        getDb().importMemorySnapshots(scopeRef.workspace, normalizedSnapshots, scopeRef.projectId)
        importedSnapshotCount += normalizedSnapshots.length
      }
    }

    _log('LEGACY_MEMORY_DB_BOOTSTRAP_DONE', {
...
export async function ensureNoteScopeReady(workspace: string, projectId?: string): Promise<void> {
  ...
  const { loadLegacyNotes, importLegacyNotesIfEmpty } = await import('../../../services/notes/notes-crud')
  const legacyNotes = await loadLegacyNotes(workspace, projectId)
  if (legacyNotes.length > 0) {
    importLegacyNotesIfEmpty({ workspace, projectId }, legacyNotes)
    _log('PROJECT_NOTES_DB_SCOPE_MIGRATED', {
      scope,
...
export async function ensureTaskMemoryScopeReady(workspace: string, projectId?: string): Promise<void> {
  ...
  const { loadLegacyTaskMemories, importLegacyTaskMemoriesIfEmpty } = await import('./store')
  ...
  if (legacyActive.length > 0 || legacyArchive.length > 0) {
    ...
    _log('TASK_MEMORY_DB_SCOPE_MIGRATED', {
      scope,
...
export async function ensureSnapshotScopeReady(workspace: string, projectId?: string): Promise<void> {
  ...
  const { loadLegacySnapshots, importLegacySnapshotsIfEmpty } = await import('./snapshot')
  ...
  if (legacySnapshots.length > 0) {
    ...
    _log('MEMORY_SNAPSHOT_DB_SCOPE_MIGRATED', {
      scope,
...
export function resolveScope(workspace: string, projectId?: string): string {
  const { projectScope, workspaceHash } = require('../paths')
