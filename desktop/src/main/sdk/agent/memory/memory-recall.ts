...
import type { ProviderKey, ProviderOverrides } from '../llm/client'
import type { ChatMessage } from '../llm/client'
import { requestStreamWithTools } from '../llm/client'
import type { ToolDefinition } from '../tools'
import type { Logger, NotesService } from '../services'
...
import { loadTaskMemoriesForRecall } from './store'
...
export function setMemoryRecallLogger(logger: Logger) { _log = logger }
export function setMemoryRecallNotes(notes: NotesService) { _notesService = notes }

function getNotes(): NotesService {
  if (!_notesService) throw new Error('NotesService not initialized for memory-recall')
...
type RecallCandidate = {
  ...
  text: string
  timestamp: number
  data: ProjectNote | TaskMemoryEntry
  score: number
  reason: string[]
}
...
async function selectRecallCandidatesByTool(
  ...
  try {
    ...
    _log('BACKGROUND_CONTEXT_TOOL_SELECTION_FAIL', { error: err instanceof Error ? err.message : String(err) }, options?.logScope)
    return null
  }
...
/* ------------------------------------------------------------------ */
/*  项目规则文件（.taco/rules/rules.md）                                 */
/* ------------------------------------------------------------------ */

const RULES_DIR = '.taco/rules'
const RULES_FILE = 'rules.md'
const OLD_RULES_PATH = '.taco/rules.md'

/**
 * 读取项目规则文件。若新路径不存在，自动从旧路径迁移。
 * 若文件不存在但 DB 中有历史手动记忆，则自动迁移到文件。
...
async function readProjectRulesOrMigrate(
  ...
  const oldRulesPath = path.join(workspace, OLD_RULES_PATH)

  // 0. 兼容迁移：新路径不存在但旧路径 .taco/rules.md 存在时，自动迁移
  try {
    const oldContent = await fs.readFile(oldRulesPath, 'utf-8')
    if (oldContent) {
      await fs.mkdir(path.join(workspace, RULES_DIR), { recursive: true })
      await fs.writeFile(rulesPath, oldContent, 'utf-8')
      _log('PROJECT_RULES_MIGRATED_FROM_OLD_PATH', { oldPath: oldRulesPath, newPath: rulesPath })
      return [{
        id: 'project-rules',
        title: '项目规则',
        content: oldContent,
  ...
  try {
    ...
    if (err.code !== 'ENOENT') {
      _log('PROJECT_RULES_READ_ERROR', { error: err.message })
      return []
    }
  }
  ...
  try {
    const dbNotes = await getNotes().list(workspace, projectId) as any[]
    ...
    _log('PROJECT_RULES_MIGRATED', { count: dbNotes.length, path: rulesPath })
    ...
    _log('PROJECT_RULES_MIGRATE_ERROR', { error: migrateErr.message })
    return []
  }
}
