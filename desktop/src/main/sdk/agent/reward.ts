...
import type { FsProvider } from './services'
...
function resolveDataHome(fsProvider: FsProvider): string {
  try {
    const userData = fsProvider.getUserDataPath()
...
function rewardDir(fsProvider: FsProvider): string {
  return path.join(resolveDataHome(fsProvider), 'reward-score')
}
...
function stateFilePath(workspace: string | undefined, projectId: string | undefined, fsProvider: FsProvider): string {
  return path.join(rewardDir(fsProvider), `${scopeKey(workspace, projectId)}.json`)
}

async function ensureRewardDir(fsProvider: FsProvider): Promise<void> {
  await fs.mkdir(rewardDir(fsProvider), { recursive: true })
}
...
async function loadState(workspace: string | undefined, projectId: string | undefined, fsProvider: FsProvider): Promise<RewardScoreState> {
  const file = stateFilePath(workspace, projectId, fsProvider)
  try {
...
async function saveState(workspace: string | undefined, projectId: string | undefined, state: RewardScoreState, fsProvider: FsProvider): Promise<void> {
  await ensureRewardDir(fsProvider)
  const file = stateFilePath(workspace, projectId, fsProvider)
...
export async function applyRewardScore(input: RewardApplyInput, fsProvider: FsProvider): Promise<RewardApplyResult> {
  const workspace = String(input.workspace ?? '').trim()
  const projectId = String(input.projectId ?? '').trim()
  const prev = await loadState(workspace, projectId, fsProvider)
  ...
  await saveState(workspace, projectId, next, fsProvider)
