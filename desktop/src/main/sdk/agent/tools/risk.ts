...
/** 模块级 DatabaseService（供 saveAuthLevel/loadAuthLevel 降级使用） */
let _db: DatabaseService | null = null
export function setRiskDatabase(db: DatabaseService) { _db = db }
...
export function saveAuthLevel(projectId: string, level: AuthLevel, db?: DatabaseService) {
  if (!projectId || !projectId.trim()) return // projectId 为空时不保存
  const key = computeProjectScope(projectId)
  const resolvedDb = db ?? _db
  if (!resolvedDb) return // 无数据库可用，跳过持久化
...
export function loadAuthLevel(projectId: string, db?: DatabaseService): AuthLevel {
