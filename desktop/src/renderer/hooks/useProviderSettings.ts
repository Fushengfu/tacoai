...
function hasLegacyProviderState(forms: ProviderForms): boolean {
  return providers.some((provider) => {
    ...
    return Boolean(form?.baseUrl || form?.apiKey || form?.model || form?.contextLength || form?.temperature)
  })
}
...
function fromLegacyProviderForms(forms: ProviderForms): ModelConfig[] {
  ...
  return providers
    .map((item, index) => {
      ...
      const hasAnyValue = Boolean(form?.baseUrl || form?.apiKey || form?.model || form?.contextLength || form?.temperature)
...
function normalizeModelConfig(raw: unknown, index: number): ModelConfig | null {
  ...
  const rawName = asText(obj.name)
  const isNameKnownProvider = rawName && providers.some(
    (p) => rawName === p.id || rawName === p.label,
  )
  const name = isNameKnownProvider
    ? (providers.find((p) => p.id === provider)?.label ?? provider)
    : (rawName || model || providers.find((p) => p.id === provider)?.label || provider)
...
export function useProviderSettings() {
  ...
  function addModelConfig(initial?: Partial<ModelConfig>): string {
    ...
    const next: ModelConfig = {
      ...
      baseUrl: asText(initial?.baseUrl) || PROVIDER_DEFAULT_BASE_URLS[provider],
  ...
  function updateModelConfig(id: string, patch: Partial<Omit<ModelConfig, 'id'>>) {
    ...
    setModelConfigs((prev) => prev.map((item) => {
      ...
      const newProviderLabel = providers.find((p) => p.id === nextProvider)?.label ?? nextProvider
      const isNameKnownProvider = providers.some(
        (p) => item.name === p.id || item.name === p.label,
      )
      const isNameAutoGenerated = !item.name || isNameKnownProvider
      const nextName = typeof patch.name === 'string'
        ? patch.name
        : (isNameAutoGenerated ? newProviderLabel : item.name)
      const nextBaseUrl = typeof patch.baseUrl === 'string'
        ? patch.baseUrl
        : (patch.provider && PROVIDER_DEFAULT_BASE_URLS[nextProvider])
          ? PROVIDER_DEFAULT_BASE_URLS[nextProvider]
          : item.baseUrl
