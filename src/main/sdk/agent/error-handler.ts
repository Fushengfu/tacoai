...
let _log: Logger = () => {}
export function setErrorHandlerLogger(logger: Logger) { _log = logger }
...
/** 创建一个确认请求并等待用户响应 */
export function waitForConfirm(confirmId: string, signal?: AbortSignal, onExpired?: () => void): Promise<boolean> {
  return new Promise((resolve) => {
    // 超时自动拒绝
    const timer = setTimeout(() => {
      _log('CONFIRM_TIMEOUT', { confirmId, timeout: CONFIRM_TIMEOUT_MS }, 'agent')
      pendingConfirms.delete(confirmId)
      onExpired?.() // 通知上层（如 loop.ts）发送事件给渲染进程清除确认 UI
...
/** 创建一个重试确认请求并等待用户响应 */
export function waitForRetry(retryId: string, signal?: AbortSignal, onExpired?: () => void): Promise<boolean> {
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      _log('RETRY_TIMEOUT', { retryId, timeout: RETRY_TIMEOUT_MS }, 'agent')
      pendingRetries.delete(retryId)
      onExpired?.() // 通知上层发送事件给渲染进程清除确认 UI
