...
import { sortTaskMemoriesByTimeAsc, mergeTaskMemoryById, loadTaskMemoriesForRecall } from './store'

/** 模块级 logger，由外部通过 setMaintainLogger 注入 */
let _log: Logger = () => {}
export function setMaintainLogger(logger: Logger) { _log = logger }
...
async function loadTaskMemories(workspace: string, projectId?: string): Promise<TaskMemoryEntry[]> {
  const { loadTaskMemoriesForRecall } = await import('./store')
...
async function loadTaskMemoryArchive(workspace: string, projectId?: string): Promise<TaskMemoryEntry[]> {
  const { loadTaskMemoryArchive } = await import('./store')
...
async function saveTaskMemories(workspace: string, items: TaskMemoryEntry[], projectId?: string): Promise<void> {
  const { saveTaskMemories } = await import('./store')
...
async function saveTaskMemoryArchive(workspace: string, items: TaskMemoryEntry[], projectId?: string): Promise<void> {
  const { saveTaskMemoryArchive } = await import('./store')
...
export async function maintainTaskMemoriesByAI(
  ...
  try {
    ...
    try {
      ...
      if (!parsedDecision) {
        _log('TASK_MEMORY_MAINTAIN_PARSE_FAIL', { reason: 'invalid_json', raw: shortText(raw, 1200) }, options?.logScope)
        return { applied: false, merged: 0, dropped: 0, total: candidates.length, reason: 'invalid_json' }
      }
    } catch (err) {
      _log('TASK_MEMORY_MAINTAIN_AI_FAIL', { error: err instanceof Error ? err.message : String(err) }, options?.logScope)
    ...
    if (remaining.length <= 0) {
      _log('TASK_MEMORY_MAINTAIN_ABORT_ALL_DELETED', {
    ...
    memoryMaintainLastRunAtByScope.set(scope, Date.now())
    _log('TASK_MEMORY_MAINTAIN_APPLIED', {
      total: candidates.length,
      mergeActions: parsedDecision.merge_actions.length,
      mergedCount,
      droppedCount,
