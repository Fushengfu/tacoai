...
import type { MemorySnapshotStore } from '../services'
...
/** 模块级 MemorySnapshotStore */
let _store: MemorySnapshotStore | null = null
export function setSnapshotStore(store: MemorySnapshotStore) { _store = store }
function getStore(): MemorySnapshotStore | null {
  return _store
}
...
export async function loadMemorySnapshots(workspace: string, projectId?: string): Promise<MemorySnapshotEntry[]> {
  ...
  if (!store) return []
  return store.listForScope({ workspace, projectId }).map((item, index) => normalizeMemorySnapshotEntry(item, index))
}

export async function saveMemorySnapshots(workspace: string, items: MemorySnapshotEntry[], projectId?: string): Promise<void> {
  const store = getStore()
  if (!store) return
  store.replaceForScope({ workspace, projectId }, items)
}
...
export async function importLegacySnapshotsIfEmpty(scopeRef: MemoryScope, legacySnapshots: MemorySnapshotEntry[]): Promise<void> {
  if (legacySnapshots.length > 0) {
    const store = getStore()
    if (!store) return
    store.importForScope(scopeRef, legacySnapshots)
  }
}
