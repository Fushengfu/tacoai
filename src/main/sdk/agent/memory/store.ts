...
export function setMemoryStoreDatabase(db: DatabaseService) { _db = db }
function getDb(): DatabaseService {
  if (!_db) throw new Error('DatabaseService not initialized for memory-store')
  return _db
}
...
export async function loadTaskMemories(workspace: string, projectId?: string): Promise<TaskMemoryEntry[]> {
  return getDb().listTaskMemoriesByTier(workspace, 'active', projectId).map((item, index) => normalizeTaskMemoryEntry(item, index))
}

export async function loadTaskMemoryArchive(workspace: string, projectId?: string): Promise<TaskMemoryEntry[]> {
  return getDb().listTaskMemoriesByTier(workspace, 'archive', projectId).map((item, index) => normalizeTaskMemoryEntry(item, index))
}

export async function saveTaskMemories(workspace: string, items: TaskMemoryEntry[], projectId?: string): Promise<void> {
  getDb().replaceTaskMemoriesByTier(workspace, 'active', items, projectId)
}

export async function saveTaskMemoryArchive(workspace: string, items: TaskMemoryEntry[], projectId?: string): Promise<void> {
  getDb().replaceTaskMemoriesByTier(workspace, 'archive', items, projectId)
}
...
export async function recordTaskLog(workspace: string, input: TaskLogInput, projectId?: string): Promise<TaskMemoryEntry | null> {
  ...
  if (normalizedSourceRef.sessionId && sourceMessageIds.length > 0) {
    const resolved = getDb().resolveChatStoreMessageSeqRange(normalizedSourceRef.sessionId, sourceMessageIds)
...
export async function importLegacyTaskMemoriesIfEmpty(
  ...
  if (legacyActive.length > 0 && !getDb().hasAnyTaskMemories(scopeRef.workspace, scopeRef.projectId)) {
    getDb().importTaskMemoriesByTier(scopeRef.workspace, 'active', legacyActive, scopeRef.projectId)
  }
  if (legacyArchive.length > 0 && !getDb().hasAnyTaskMemories(scopeRef.workspace, scopeRef.projectId)) {
    getDb().importTaskMemoriesByTier(scopeRef.workspace, 'archive', legacyArchive, scopeRef.projectId)
  }
}
